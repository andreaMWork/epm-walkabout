contract gendoug {
	...
	function permExist(bytes32 permName) returns (uint ret) {
	...
	function getPerm(bytes32 permName, address addr) returns (uint ret) {
		...
		if (u.permvals[p.permnum] == 0){
			if (u.exclude[p.permnum] == 1) return u.permvals[p.permnum];
	...
	function setPerm(bytes32 permName, address addr, uint value) returns (uint ret) {
		permdata p = perms [permName];
		userdata u = users[addr];
